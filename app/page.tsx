...
import { buildImageFallback } from "@/lib/image";
...
export default function Home() {
      ...
      setMessages([
        {
          ...
          role: "assistant",
          text: `Post em rascunho carregado: ${parsed.titulo}`,
        },
      ]);
    } catch {
  ...
  const handleGeneratePost = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = generatedPost
      ? { instruction: cleanedPrompt, currentPost: generatedPost }
    ...
    try {
      ...
      if (!response.ok) {
        const message = "error" in data ? data.error : "Não foi possível gerar a sugestão neste momento.";
        setError(message);
        appendMessage("assistant", `Não consegui processar: ${message}`);
        return;
      }
      ...
      appendMessage(
        "assistant",
        generatedPost
          ? "Atualizei a postagem com seus ajustes. Pode continuar pedindo alterações."
          : "Postagem criada! Se quiser, peça mudanças de legenda, arte ou horário.",
      );
      setPrompt("");
    } catch {
      setError("Falha de conexão ao gerar/refinar a sugestão. Tente novamente.");
      appendMessage("assistant", "Tive uma falha de conexão agora. Tenta de novo em alguns segundos.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleQuickCaptionRegeneration = async () => {
    setPrompt("Ajusta só a legenda mantendo tema, arte e hashtags.");
  };
  ...
  const handleRegenerateImage = async () => {
    ...
    try {
      ...
      appendMessage("assistant", "Gerei uma nova versão da arte para você.");
    } catch {
  ...
  const handleApproveAndSchedule = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        appendMessage("assistant", "Post agendado em modo local (localStorage). Configure Supabase para produção.");
        return;
      }

      setScheduledPosts((prev) => [data as ScheduledPost, ...prev]);
      appendMessage("assistant", "Perfeito, post aprovado e agendado ✅");
      localStorage.removeItem(draftStorageKey);
      setGeneratedPost(null);
      setSelectedScheduleAt("");
    } catch {
  ...
  const handleMetaConnect = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      appendMessage("assistant", "Conta Meta conectada. Use 'Publicar agora' nos cards do planejador.");
    } catch {
    ...
    <main className="page">
      <section className="content">
        <header className="topArea">
          <div className="conversationTop">
            ...
            {(generatedPost || messages.length > 0) && (
              <button type="button" className="newPostBtn" onClick={resetConversation}>
                <RefreshCcw size={14} /> Criar nova postagem
              </button>
            )}
          </div>
          ...
          <form className="chatBar" onSubmit={handleGeneratePost}>
            <input
              ...
              placeholder={
                generatedPost
                  ? "Peça ajustes diretos: legenda, arte, CTA, etc..."
                  : "Ex: cria uma postagem para o Dia dos Pais"
              }
            />
          ...
          {generatedPost ? (
            <section className="generatedCard morphIn" aria-live="polite">
              ...
              <img
                ...
                loading="lazy"
                onError={(event) => {
                  event.currentTarget.src = buildImageFallback(generatedPost.promptVisual);
                }}
              />
      ...
      <aside className="rightPanel">
        ...
        <section className="card eventCard">
          <h2>Conexão Meta (configura aqui)</h2>
        ...
        <section className="aiTipCard">
          ...
          <p>“Depois de gerar, continue conversando no mesmo campo para ajustar arte, legenda e CTA.”</p>
        </section>

        <section className="notificationCard">
          <MessageCircle size={16} />
          Fluxo completo: gerar, conversar, aprovar, agendar e publicar manualmente.
        </section>
